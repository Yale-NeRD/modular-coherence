...
from protocol.mosi.cache_state_mosi import *
from framework.requestor.requestor import requestor
import textwrap
import inspect
...
class requestor_mosi(requestor):
    ...
    def __init__(self, interconnect, requestor_arch, local_cache_state, name):
        super(requestor_mosi, self).__init__(
            interconnect, requestor_arch, name)

        # local cache state passed in at init time in user defined tests
        ...
        self.match_action_table["write"][SHARED] = [
            (self.send_invalidation_to_dir, ['getM'])]

        self.match_action_table["read"][INVALID] = [
            (self.send_invalidation_to_dir, ['getS'])]
        self.match_action_table["write"][INVALID] = [
            (self.send_invalidation_to_dir, ['getM'])]

        self.match_action_table["read"][OWNED] = []
        self.match_action_table["write"][OWNED] = [
            (self.send_invalidation_to_dir, ['getM'])]

        self.match_action_table["change_state"][MODIFIED] = [
            (self.update_state, [])]
        self.match_action_table["change_state"][INVALID] = [
            (self.update_state, [])]
        self.match_action_table["change_state"][SHARED] = [
            (self.update_state, [])]
    ...
    def get_current_state(self, memory_addr):
        ...
        """
        current_state = self.requestor_arch.get_current_cache_line_mode(
            memory_addr)
    ...
    def send_invalidation_to_dir(self, message_new, dest, src, msg_name, memory_addr, message):
        ...
        self.interconnect.send_message(
            'directory', self.name, (message_new, memory_addr))
    ...
    def update_state(self, dest, src, msg_name, memory_addr, message):
        ...
        self.requestor_arch.update_cache_state(
            memory_addr, mode, new_mode_value)
