...
from framework.invalidator.invalidator import invalidator
import textwrap
import inspect
...
class invalidator_mosi(invalidator):
    def __init__(self, interconnect, invalidator_arch, local_cache_state, name="a"):
        super(invalidator_mosi, self).__init__(
            interconnect, invalidator_arch, name)
    ...
    int
        ...
        """
        current_state = self.invalidator_arch.get_stored_cache_line_mode(
            memory_addr)
    ...
    memory_addr: int - address of block being requested 
    message: obj - full message that was sent 
    ...
    None
        ...
        self.interconnect.send_message(
            'directory', self.name, ("ACK", memory_addr))
    ...
    None
        ...
        self.invalidator_arch.update_cache_state(
