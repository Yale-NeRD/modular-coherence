class run_entity(object):
    def __init__(self, interconnect, invalidator_bool):
        ...
        # tells interconnect whether to read from invalidator, requestor, or directory; IF true, reads from invalidator queue, if False, reads requestor queue; if None, reads directory queue (if self.name is "directory" too)
    ...
    def run(self, thread_return=None):
        message = self.interconnect.get_message(
            self.name, invalidator=self.invalidator)
